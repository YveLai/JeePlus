	...
	export default {
		...
		async mounted () {
			  ...
			  // 读取流程表单
				if (this.formUrl === '/404') {
				  ...
				  let {data} = await this.formService.getMobileForm(this.formUrl);
				 // 初始化动态表单
				  ...
				  if (this.status === 'start') {
				    // 读取启动表单配置
				    let {data} = await this.formService.getStartFormData({processDefinitionId: this.procDefId})
				    this.setData(data, 'start')
				  } else {
				    // 读取任务表单配置
				    let {data} = await this.formService.getTaskFormData({taskId: this.taskId})
		...
		methods:{
			...
			selectUsersToAddSignTask (users) {
			  ...
			  this.taskService.addSignTask({taskId: this.taskId, userIds: JSON.stringify(userIds), message: '', flag: false}).then(({data}) => {
			...
			selectUsersToTransferTask (userId) {
			  ...
			  this.taskService.transfer({taskId: this.taskId, userId: userId}).then(({data}) => {
				uni.showToast({ title: data, icon: "success" });
				uni.navigateTo({
					url: '/pages/workbench/task/TodoList'
				})
			...
			selectUsersToDelateTask (userId) {
			  ...
			  this.taskService.delegate({taskId: this.taskId, userId: userId}).then(({data}) => {
				uni.showToast({ title: data, icon: "success" });
				uni.navigateTo({
					url: '/pages/workbench/task/TodoList'
				})
			...
			stop () {
				...
				uni.showModal({
				    ...
				    success: (res) => {
				        if (res.confirm) {
							this.processService.stop({id: this.procInsId, ...this.auditForm}).then(({data}) => {
				              uni.showToast({ title: data, icon: "success" });
				              uni.navigateTo({
				              	url: '/pages/workbench/task/TodoList'
				              })
